...
const getCurrentState = (pageNumber = 1) => {
...
window.refreshFilters = debounce(function refreshFilters(pageNumber = 1) {
    ...
    const state = getCurrentState(pageNumber);
    const pageChanged = window.PAGE_NUMBER && window.PAGE_NUMBER !== state.page;
    ...
    // when the page number changes, we push a new state, otherwise
...
$(function () {
    ...
    history.replaceState(state, null, getFilterString(state));
});
