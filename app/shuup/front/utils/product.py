...
youtube_re_link = re.compile(r"(https?://)?(www\.)?((youtu\.be/)|(youtube\.com/watch/?\?v=))([A-Za-z0-9-_]+)", re.I)
youtube_embed = "https://www.youtube.com/embed/{}"
vimeo_re_link = re.compile(
    r"(?:https?:\/\/(?:www\.)?)?vimeo.com\/(?:channels\/|groups\/([^\/]*)\/videos\/|album\/(\d+)\/video\/|)(\d+)(?:$|\/|\?)",  # noqa: E501
    re.I,
)
vimeo_embed = "https://player.vimeo.com/video/{0}"
...
def get_embed_youtube(video_link):
    return youtube_re_link.sub(lambda match: youtube_embed.format(match.groups()[5]), video_link)


def get_embed_vimeo(video_link):
    return vimeo_re_link.sub(lambda match: vimeo_embed.format(match.groups()[2]), video_link)
