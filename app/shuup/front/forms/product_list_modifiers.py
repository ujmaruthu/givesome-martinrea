...
class AttributeProductListFilter(SimpleProductListModifier):
    ...
    def _build_attribute_filter_fields(
        ...
        for attribute in attributes:
            field_key = f"attr_{attribute.pk}"
            ...
            if attribute.type == AttributeType.CHOICES and attribute.choices.exists():
                ...
                fields.append((field_key, forms.FloatField(required=False, label=attribute.name)))
            elif attribute.type == AttributeType.INTEGER:
                fields.append((field_key, forms.IntegerField(required=False, label=attribute.name)))
            elif attribute.type in (AttributeType.UNTRANSLATED_STRING, AttributeType.TRANSLATED_STRING):
                fields.append((field_key, forms.CharField(required=False, label=attribute.name)))
    ...
    def _get_attributes_from_shop_config(self, shop):
        ...
        if filterable_attribute_pks:
            attributes = Attribute.objects.filter(pk__in=filterable_attribute_pks)
    ...
    def _get_product_attribute_query_strings(self, request, data, category=None) -> Dict[Attribute, Any]:
        """
        ...
        if category:
            valid_attributes = self._get_attributes_from_category(request.shop, category)
        else:
            valid_attributes = self._get_attributes_from_shop_config(request.shop)

        valid_attributes = dict([(attribute.pk, attribute) for attribute in valid_attributes])

        attributes = {}
        for key, value in data.items():
            if not key.startswith("attr_") or value is None:
                continue

            attr_pk = int(key.split("attr_")[1])
            attribute = valid_attributes.get(attr_pk)
    ...
    def get_products_queryset(self, request, queryset, data):  # noqa: C901
        # Filter for chosen attributes
        attributes = self._get_product_attribute_query_strings(request, data)
        if not attributes:
            return queryset
        ...
        for attribute, value in attributes.items():
            ...
            # attribute is a choice value
            if attribute.type == AttributeType.CHOICES:
                if isinstance(value, str):
                    value = value.split(",")

                qs_filter = Q(attributes__attribute=attribute, attributes__chosen_options__in=value)

            elif attribute.type in (AttributeType.INTEGER, AttributeType.DECIMAL):
                qs_filter = Q(attributes__attribute=attribute, attributes__numeric_value=value)

            elif attribute.type == AttributeType.BOOLEAN and len(value):
                qs_filter = Q(attributes__attribute=attribute, attributes__numeric_value=int(bool(int(value))))

            elif attribute.type == AttributeType.UNTRANSLATED_STRING:
                qs_filter = Q(attributes__attribute=attribute, attributes__untranslated_string_value__icontains=value)

            elif attribute.type == AttributeType.TRANSLATED_STRING:
                qs_filter = Q(
                    attributes__attribute=attribute, attributes__translations__translated_string_value__icontains=value
                )
