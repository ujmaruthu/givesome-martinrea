...
from shuup.core.excs import UnavailableError
...
class MethodsForm(forms.Form):
    ...
    def clean_shipping_method(self):
        ...
        if shipping_method and shipping_method not in self.basket.get_available_shipping_methods():
            raise ValidationError(_("Invalid shipping method."))
        return shipping_method

    def clean_payment_method(self):
        payment_method = self.cleaned_data.get("payment_method")
        if payment_method and payment_method not in self.basket.get_available_payment_methods():
    ...
    def limit_method_fields(self):
        ...
        for field_name, methods in (
            ("shipping_method", basket.get_available_shipping_methods(validate_unavailability_reasons=False)),
            ("payment_method", basket.get_available_payment_methods(validate_unavailability_reasons=False)),
        ):
            ignore_methods = []
            for method in methods:
                unavailability_errors = []

                for unavailability_reason in method.get_unavailability_reasons(basket):
                    if isinstance(unavailability_reason, ValidationError):
                        ignore_methods.append(method)
                        unavailability_errors = []
                        break

                    if isinstance(unavailability_reason, UnavailableError):
                        unavailability_errors.append(str(unavailability_reason.message))

                if unavailability_errors:
                    method.enabled = False

                method.errors = unavailability_errors

            field = self.fields[field_name]
            mci = MethodChoiceIterator(field)
            field.choices = [mci.choice(obj) for obj in methods if obj not in ignore_methods]
...
class MethodsPhase(CheckoutPhaseViewMixin, FormView):
    ...
    def is_valid(self):
        ...
        if shipping_required and self.basket.has_shippable_lines() and not self.storage.get("shipping_method_id"):
            return False
