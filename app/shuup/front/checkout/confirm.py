...
class ConfirmPhase(CheckoutPhaseViewMixin, FormView):
    ...
    def is_valid(self):
        required_fields = self.storage.get("required_fields")

        # this was not set, hence, not valid
        if required_fields is None:
            return False

        # check that all form keys that should be required
        invalid_values = [key for key in self.storage.keys() if required_fields and not self.storage.get(key)]
        return bool(len(invalid_values) == 0)
    ...
    def get_context_data(self, **kwargs):
        ...
        context["provider_fields"] = []

        for provider_cls in get_provide_objects("checkout_confirm_form_field_provider"):
            provider = provider_cls()
            for definition in provider.get_fields(request=self.request):
                context["provider_fields"].append(definition.name)
    ...
    def form_valid(self, form):
        ...
        # set the required fields into the storage
        required_fields = [name for name, field in form.fields.items() if field.required]
        self.storage["required_fields"] = required_fields

        self.process()

        # save the confirm form data into extra data
        self.basket.extra_data["confirmed_data"] = form.cleaned_data
