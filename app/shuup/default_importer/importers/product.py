...
class ProductMetaBase(ImportMetaBase):
    ...
    def _handle_image(self, shop, product, image_source, is_primary=False, image_ordering=0):
        if urlparse(image_source).scheme in ("http", "https"):
            r = requests.get(image_source, allow_redirects=True)
            filename = urlparse(image_source).path.split("/")[-1]
            with NamedTemporaryFile("w+b", delete=True) as img_temp:
                img_temp.write(r.content)
                img_temp.flush()
                img, _ = Image.objects.get_or_create(name=filename, defaults=dict(file=File(img_temp)))
                media_file = MediaFile.objects.create(file=img)
    ...
    def handle_images(self, fields, sess):
        ...
        for image_ordering, image_field in enumerate(self.aliases["image"]):
            image = row.get(image_field)
            if image and not self._handle_image(
                sess.shop, product, row[image_field], is_primary=True, image_ordering=image_ordering
            ):
                msg = _("Image '%s' was not found, please check whether the image exists.") % row[image_field]
                sess.log_messages.append(msg)

        for image_ordering, image_field in enumerate(self.aliases["media"]):
            images = row.get(image_field)
            if images:
                for image_source in images.split(","):
                    if not self._handle_image(sess.shop, product, image_source, image_ordering=image_ordering):
