...
class Order(MoneyPropped, models.Model):
    ...
    def product_ids(self):
        return self.lines.products().values_list("product__id", flat=True)
    ...
    def set_canceled(self):
        if self.status.role != OrderStatusRole.CANCELED:
            self.change_status(next_status=OrderStatus.objects.get_default_canceled())
    ...
    def is_international_shipping(self):
        return not (self.supplier_order.supplier.contact_address.country.code == self.shipping_address.country.code)
    ...
    def get_product_summary(self, supplier=None):
        ...
        for product_id, quantity in self._get_shipped_quantities(supplier_id):
            ...
            products[product_id]["unshipped"] = max(products[product_id]["unshipped"] - quantity, 0)
    ...
    def get_available_shipping_methods(self, validate_unavailability_reasons=True):
        ...
        suppliers_from_lines = set(
            self.lines.filter(product__isnull=False, supplier__isnull=False)
            .values_list("supplier", flat=True)
            .distinct()
        )
        queryset = (
            ShippingMethod.objects.available(shop=self.shop, products=product_ids)
            .filter(Q(supplier__in=suppliers_from_lines) | Q(supplier__isnull=True))
            .distinct()
        )

        queryset = ShopProduct.filter_limited_services(queryset, self.shop, product_ids, "shipping_methods")
    ...
    def get_available_payment_methods(self, validate_unavailability_reasons=True):
        ...
        product_ids = self.product_ids
        queryset = PaymentMethod.objects.available(shop=self.shop, products=product_ids)
        queryset = ShopProduct.filter_limited_services(queryset, self.shop, product_ids, "payment_methods")
