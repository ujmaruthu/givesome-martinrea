...
if TYPE_CHECKING:
    from shuup.core.models._contacts import Contact
    from shuup.core.models._service_base import Service
...
class ShopProductVisibility(Enum):
    ...
    class Labels:
        ...
        ONLY_VISIBLE_WHEN_PROMOTED = _("only visible when promoted")
...
class ShopProduct(MoneyPropped, TranslatableModel):
    ...
    product = UnsavedForeignKey(
        ...
        help_text=_("Related product ID"),
    )
    ...
    def get_visibility_errors(self, customer: "Contact" = None):  # noqa: C901
        if self.product.deleted:
        ...
        # only do the following checks if the customer is not None
        from shuup.core.models._contacts import Contact
    ...
    def images(self):
        return self.product.media.filter(
            shops=self.shop, kind__in=[ProductMediaKind.IMAGE, ProductMediaKind.VIDEO]
        ).order_by("ordering")
    ...
    @classmethod
    def filter_limited_services(
        cls,
        queryset: "QuerySet[Service]",
        shop: "Shop",
        products_ids: Iterable[int],
        service_property_name: str,
    ):
        """
        Filter out services from the `queryset` that are not part
        of the shop products service limitation.

        `service_property_name` can be either `shipping_methods` or `payment_methods`
        """
        assert service_property_name in ("shipping_methods", "payment_methods")

        # check for products that have limited service
        filters = {
            "shop": shop,
            "product__in": products_ids,
            f"limit_{service_property_name}": True,
            f"{service_property_name}__isnull": False,
        }
        shop_products_with_limited_services = list(
            ShopProduct.objects.only(service_property_name)
            .prefetch_related(service_property_name)
            .filter(**filters)
            .distinct()
        )
        common_services = None
        has_limited_services = False

        for shop_product in shop_products_with_limited_services:
            has_limited_services = True
            services_manager = getattr(shop_product, service_property_name)
            services_ids = set(list(services_manager.values_list("pk", flat=True)))

            if not services_ids:
                continue

            if common_services is None:
                common_services = services_ids
            else:
                common_services &= common_services

        # there are shop products that have services with limitations
        if has_limited_services:
            # there are common services for all products, then filter
            # the queryset to only return the common ones
            if common_services:
                queryset = queryset.filter(pk__in=common_services)

            # there is no common service, return a blank queryset :/
            else:
