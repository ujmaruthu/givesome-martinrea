...
@library.filter
def safe_vendor_description_video(value):
    try:
...
@library.filter
def safe_vendor_description_img(value):
    try:
        soup = BeautifulSoup(value)
        src_attribute = soup.find("img")["src"]
    except:
        src_attribute = False
    return src_attribute

@library.filter
def safe_vendor_description_title(value):
    try:
        soup = BeautifulSoup(value)
        src_attribute = soup.find("label")
        src_attribute = src_attribute.string
    except:
        src_attribute = False
    return src_attribute
@library.filter
def safe_vendor_description_video_title(value):
    try:
        soup = BeautifulSoup(value)
        src_attribute = soup.find("strong")
        src_attribute = src_attribute.string
    except:
        src_attribute = False
    return src_attribute
@library.filter
def safe_vendor_description_content(value):
    # result = re.sub("<.*?>", "", value)
    # return mark_safe(result)
    try:
        soup = BeautifulSoup(value)
        src_attribute = soup.find("p")
        src_attribute = src_attribute.string
    except:
        src_attribute = False
    return src_attribute
