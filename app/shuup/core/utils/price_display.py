...
from shuup.core.utils.price_cache import (
    _get_price_info_cache_key_params,
...
class MaxPricePercentPropertyFilter(_ContextFilter):
    def __call__(self, context, item, quantity=1, allow_cache=True, supplier=None):
        request = context.get("request")

        # if this is a parent, check whether children have discounts
        if isinstance(item, Product) and item.mode in (
            ProductMode.VARIABLE_VARIATION_PARENT,
            ProductMode.SIMPLE_VARIATION_PARENT,
        ):
            if allow_cache:
                cache_key_context = _get_price_info_cache_key_params(context, item, quantity, supplier=supplier)
                cache_key_context["identifier"] = "max_price_percentage"
                cache_key, max_price_percentage = context_cache.get_cached_value(**cache_key_context)

                if max_price_percentage is not None:
                    return percent(max_price_percentage)
            ...
            product_queryset = catalog.get_products_queryset().filter(
                pk__in=item.variation_children.values_list("pk", flat=True)
            )

            percentage = 0
            for child in product_queryset:
                child_price_info = _get_item_price_info(request, child, quantity, supplier)
                percentage = max(percentage, getattr(child_price_info, self.property_name))

            if allow_cache:
                context_cache.set_cached_value(cache_key, percentage)
            ...
            percentage = getattr(price_info, self.property_name)

            return percent(percentage)


class DiscountedPricePropertyFilter(_ContextFilter):
    def __call__(self, context, item, quantity=1, allow_cache=True, supplier=None):
        request = context.get("request")
        ...
        if isinstance(item, Product) and item.mode in (
            ...
            if allow_cache:
                cache_key_context = _get_price_info_cache_key_params(context, item, quantity, supplier=supplier)
                cache_key_context["identifier"] = "is_discounted"
                cache_key, is_discounted = context_cache.get_cached_value(**cache_key_context)
            ...
            is_discounted = (
                ...
                .filter(pk__in=item.variation_children.values_list("pk", flat=True))
                .filter(catalog_discounted_price__gt=0)
                .exists()
            )

            if allow_cache:
                context_cache.set_cached_value(cache_key, is_discounted)
            ...
            return getattr(price_info, self.property_name) or False
