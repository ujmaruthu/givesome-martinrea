...
class BasketValidationError(ValidationError):
    """
    Represents a basket validation error that can optionally not block the checkout flow.
    It will indeed block the creation of an order.
    A non blocking checkout error represents a warning while in the checkout
    flow and an error when creating an order.
    """

    blocks_checkout = False

    def __init__(self, message, code=None, params=None, blocks_checkout=True):
        self.blocks_checkout_flow = blocks_checkout
...
class BaseBasket(OrderSource):
    ...
    def get_available_shipping_methods(self, validate_unavailability_reasons=True):
        """
        Get available shipping methods.

        :rtype: list[ShippingMethod]
        """
        suppliers_from_lines = set()
        products_ids = set()

        for line in self.get_lines():
            if not line.product or not line.supplier:
                continue

            products_ids.add(line.product.pk)
            suppliers_from_lines.add(line.supplier)

        queryset = (
            ShippingMethod.objects.available(shop=self.shop, products=self.product_ids)
            .filter(Q(supplier__in=suppliers_from_lines) | Q(supplier__isnull=True))
            .distinct()
        )

        queryset = ShopProduct.filter_limited_services(queryset, self.shop, products_ids, "shipping_methods")

        if validate_unavailability_reasons:
            return [method for method in queryset if method.is_available_for(self)]

        return queryset

    def get_available_payment_methods(self, validate_unavailability_reasons=True):
        """
        Get available payment methods.

        :rtype: list[PaymentMethod]
        """
        queryset = PaymentMethod.objects.available(shop=self.shop, products=self.product_ids)
        products_ids = set()
        ...
        queryset = ShopProduct.filter_limited_services(queryset, self.shop, products_ids, "payment_methods")
