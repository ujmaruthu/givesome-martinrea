...
class ProductHighlightPlugin(TemplatedPlugin):
    ...
    cacheable = True
    cache_namespace = "product_highlight_cell"
    fields = [
...
class ProductCrossSellsPlugin(TemplatedPlugin):
    ...
    cacheable = True
    cache_namespace = "product_cross_sells_cell"
...
class ProductsFromCategoryPlugin(TemplatedPlugin):
    ...
    cacheable = True
    cache_namespace = "category_products_highlight_cell"
    fields = [
        ("title", TranslatableField(label=_("Title"), required=False, initial="")),
        ("count", forms.IntegerField(label=_("Count"), min_value=1, initial=4)),
        (
...
class ProductSelectionPlugin(TemplatedPlugin):
    ...
    cacheable = True
    cache_namespace = "product_selection_cell"
    ...
    def get_context_data(self, context):
        ...
        if products:
            ...
            products_qs = catalog.get_products_queryset().filter(
                pk__in=products,
                mode__in=ProductMode.get_parent_modes(),
