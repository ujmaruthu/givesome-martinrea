...
from shuup.admin.utils.urls import BaseAdminURLGetter, NoModelUrl, manipulate_query_string
...
def model_url(context, model, kind="detail", default=None, **kwargs):
    ...
    try:
        ...
        admin_model_url_getters = get_provide_objects("admin_model_url_getter")
        admin_model_url_resolvers = get_provide_objects("admin_model_url_resolver")
        if admin_model_url_resolvers:
            warnings.warn(
                "Warning! `admin_model_url_resolver` is deprecated in Shuup 4.0. "
                "Use the class-based `admin_model_url_getter` instead.",
                DeprecationWarning,
            )

        for getter in sorted(admin_model_url_getters, key=lambda x: x.ordering):
            if not issubclass(getter, BaseAdminURLGetter):
                continue
            url = getter.get_model_admin_url(model, kind=kind, user=user, shop=shop, **kwargs)
            if url:
                return url
