    ...
    def __init__(self, **kwargs):
        ...
        if self.allowed_media_kinds:
            ...
            self.fields["kind"].initial = (
                self.instance.kind
                if self.instance and self.instance.kind in self.allowed_media_kinds
                else self.allowed_media_kinds[0]
            )
...
class ProductImageMediaForm(BaseProductMediaForm):
    ...
    @property
    def is_image(self):
        return self.instance.kind == ProductMediaKind.IMAGE
...
class ProductImageMediaFormSet(ProductMediaFormSet):
    allowed_media_kinds = [ProductMediaKind.IMAGE, ProductMediaKind.VIDEO]
